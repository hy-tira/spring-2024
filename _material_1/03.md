...
The topic of this and the following two chapters is the design of efficient algorithms. The goal is algorithms that execute efficiently even when the input size $$O(n)$$ is big.
...
This is still not fast enough, because computing `right_sum` is still slow, and the the trick we used for `left_sum` does not work for `right_sum`. Even with the faster computation of `left_sum`, the time complexity is still $$O(n^2)$$.
...
For example, the count at the position 5 is 3, because the sublists ending at the position 5 include $$[3,2,2]$$, $$[3,3,2,2]$$ and $$[2,3,3,2,2]$$.
...
In the example, the case 1 occurs, when `i` moves to the position 6, and the case 2 occurs when 'i' moves to the position 7. The variables `a` and `b` are updated as follows:
