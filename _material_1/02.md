...
The memory of a computer consists of a sequence of memory locations capable of storing data. Each memory location has an address that can be used for access. The memory stores the data processed by a program during its execution.
...
The list described in the chapter is more generally known as an _array list_ or a _dynamic array_.
