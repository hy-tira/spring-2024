...
Here the key function is `abs` (abstract value), which causes the sorting to ignore the minus signs.
...
C++ has the function `std::sort` that is given an iterator to the beginning and the end of the interval to be sorted. The function is used as follows:
