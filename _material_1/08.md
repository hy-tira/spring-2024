...
A feature of depth-first search is that it visits nodes in the order of their distance from the start node. Thus we can use depth-first search to determine the distance of each node to the start node, and to find a shortest path between two nodes.
