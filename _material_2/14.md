...
Each line shows one update to a distance. On the first line, the distance of the node $$2$$ is updated to $$8$$. The update uses the edge $$1 \rightarrow 2$$ of weigth $$8$$. Since the distance to the node $$1$$ is $$0$$, the new distance to the node $$2$$ is $$8$$, which is smaller than the old distance $$\infty$$.
...
When the algorithm processes the middle node $$k$$, it finds the shortest paths that contains the node $$k$$ as an intermediate node and where all the other intermediate nodes are in the range $$1 \dots k-1$$. Since $$k$$ goes through all values $$1,2,\dots,n$$, the distances at the end are the lengths of the shortest of all paths.
