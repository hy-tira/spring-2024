...
Notice that the second augmenting path includes the edge $$3 \rightarrow 2$$, which was not on the original graph. Using this edge became possible when the edge gained a positive residual capacity in the first stage. Increasing the flow through such a reverse edge corresponds to reducing the flow through the original edge, i.e., the second stage cancels a part of the flow that the first stage added for that edge.
...
Let us consider an undirected graph that is _bipartite_, which means the nodes of the graph can be divided into two groups so that every edge in the graph connects two nodes in different groups.
